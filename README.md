...
The source file has 412 system records across 43 federal organizations, with 24 columns delivered bilingually (English/French column pairs, plus one combined `government_organization` field of the form `"English name / French name"`). I split that combined field on `" / "` — verified safe across all 412 rows and all 43 distinct organization names — and dropped the French-language columns, producing a 15-column, English-only working file (`gc_ai_register_mvp_english.csv`). All other values were kept exactly as published, including inconsistent casing (`"In production"` vs. `"in production"`) and stray whitespace in categorical fields — see `data_notes.md` for the full list of data-quality observations from that step.
...
The remaining 18 gap systems are split across "In development" (12), "Retired" (4), and blank status (2). The full list of all 31, with register IDs, is in `gap_systems.csv` so every row here can be checked directly against the source file.
...
```
gc_ai_register_mvp_english.csv — English-only extract of the source CSV (412 rows, 15 columns)
data_notes.md                  — data-quality notes from the English-language extraction step
gap_systems.csv                — the 31 gap-case rows referenced in Section 3, in full
```

Every number in Section 3 can be reproduced by cross-tabulating `involves_personal_information` against `notification_ai` in `gc_ai_register_mvp_english.csv`, treating blank as its own category.
